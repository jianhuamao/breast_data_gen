...
class LoRAAttnProcessor2_0(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, rank=4, network_alpha=None):
        ...
        self.to_q_lora = [LoRALinearLayer(hidden_size, hidden_size, rank, network_alpha) for _ in range(self.frames)]
        self.to_k_lora = [LoRALinearLayer(cross_attention_dim or hidden_size, hidden_size, rank, network_alpha) for _ in range(self.frames)]
        self.to_v_lora = [LoRALinearLayer(cross_attention_dim or hidden_size, hidden_size, rank, network_alpha) for _ in range(self.frames)]
        self.to_out_lora = [LoRALinearLayer(hidden_size, hidden_size, rank, network_alpha) for _ in range(self.frames)]
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, scale=1.0):
        ...
        #     hidden_states = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        # blend;;bg;fg;
        hidden_states= einops.rearrange(hidden_states, '(b f) d c -> f b d c', f=self.frames)
        if encoder_hidden_states is not None:
            encoder_hidden_states = einops.rearrange(encoder_hidden_states, '(b f) d c -> f b d c', f=self.frames)
        h=hidden_states
        ...
        for i in range(self.frames):
            ...
            # hidden_states = F.scaled_dot_product_attention(
            #     query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False,i=i
            # )
            hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
            hidden_states = hidden_states.to(query.dtype)

            # linear proj
            hidden_states = attn.to_out[0](hidden_states) + scale * self.to_out_lora[i](hidden_states)
            # dropout
            hidden_states = attn.to_out[1](hidden_states)
            attn_outs.append(hidden_states)
